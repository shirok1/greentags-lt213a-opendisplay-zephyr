...
对齐配置的链接结果见构建脚本输出（约 96.3 kB Flash、16,336 B RAM）。此前实机验证构建为 95,788 B Flash。
RAM 已含静态线程栈和 BLE 缓冲区，未分配空间仅 48 B；这不是栈余量。
...
| `0x43/0x44` | 固件版本 0.3.0（含构建提交 SHA）、16 B MSD |
...
此前 95,788 B 构建实测栈余量为主线程252 B、系统工作队列380 B。新遥测路径仍需重测栈水位，总静态RAM未增加。
尚未完成：实际断电时 Flash 行为、所有路径的最坏栈占用、整板电流和长时间独立供电稳定性。
主业务循环喂180秒硬件看门狗，致命异常自动冷重启；详见 [功耗与稳定性](power-stability.md)。
...
与 Mynewt 对齐：启动及每次断连后 **30 秒使用 100–150 ms**，随后使用固定
**1000 ms**（另有 BLE 随机延时），TX 为 0 dBm。连接期间不执行广播切换，
断连重新开启完整快速阶段。临时 HCI 错误每 5 秒重试，保留原快速阶段期限。
扫描建议持续至少 8 秒。
...
取消一分钟广播心跳；在启动、快速阶段结束、未连接每五分钟，以及通过长度与
配置认证检查的 `00 44` 请求时，采样温度和内部 ADC VDD。采用 OpenDisplay
半度温度与 10 mV、9 位电压编码，并保留其他 MSD 状态位；采样失败保留完整
旧快照，请求路径返回 NACK。VDD 只是电池电压代理，不表示剩余容量。
ADC 成功或 50 ms 超时后停止并禁用；温度通过 Zephyr 共享驱动避免与 RC 校准
争用，正常采样后停止 TEMP。该驱动等待若卡死，由 180 秒看门狗恢复。
五分钟遥测不改变喂狗期限：主循环独立最多等待 60 秒。
...
默认在 Power Off 成功后发 `07 A5` 深睡。需要仅关电比较版本时：

```sh
uv run --locked python scripts/build.py --no-epd-deep-sleep
```

默认深睡会断开 BUSY 输入缓冲；下一次上传恢复 BUSY 输入并硬件 RESET。
需在实际 T5 上验证指令兼容、全屏/局部刷新、连续唤醒和电流。
再次运行不带此选项的构建会恢复默认深睡配置。
...
| 官方实现 | 本项目处理 |
|---|---|
| Bluefruit 快/慢广播 160 ms / 1000 ms，启动 fast timeout 10 s | 保留两阶段；跨固件配置改为 100–150 ms / 1000 ms，启动及断连后 fast timeout 30 s |
...
广播间隔是发现延迟与无线耗电的折中，当前值与 Mynewt 对齐。
