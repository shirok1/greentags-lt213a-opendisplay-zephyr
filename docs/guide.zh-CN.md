...
局部刷新 LUT 参考 [GxEPD2 T5D 驱动](https://github.com/ZinggJM/GxEPD2/blob/master/src/epd/GxEPD2_213_T5D.cpp)，
**尚未在本块 T5 屏幕上验证，不保证 T5/T5D 波形兼容**。
...
认证算法与独立 Python/OpenSSL 实现交叉验证；尚未验证官方客户端的实机握手。
...
上传方向/棋盘测试图。此辅助工具仅支持未认证的普通/压缩上传，每次 18 B 并等待 ACK。
...
尚未完成：真实 BLE/客户端互通、屏幕及局部刷新波形、实际断电时 Flash 行为、
最坏栈占用、功耗和长时间稳定性。推荐依次验证方向测试图、全白/全黑、连续上传、
传输中断线重连、BUSY 卡低、认证及配置断电恢复，最后验证局部刷新。
