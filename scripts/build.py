...
def main():
    ...
    parser.add_argument("--epd-deep-sleep", action="store_true", help="Experimental T5 panel deep sleep; requires hardware validation")
    ...
    run("west", "build", "-b", "lt213a/nrf51822", "-d", str(ROOT / "build"),
        *( ["-p", "always"] if args.pristine else [] ), str(ROOT), "--",
        "-DEXTRA_CONF_FILE=" + (str(ROOT / "epd-deep-sleep.conf") if args.epd_deep_sleep else ""), env=env)
