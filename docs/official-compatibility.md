...
[认证互通研究](auth-interoperability.md) 已用真实 SDK 与 C 实现离线复现正常握手，以及具体的错误帧不兼容；错误响应不再仅是待核对疑点。另见 [功耗与稳定性研究](power-stability.md)。研究后的修复与实机验证记录见两份文档的修复章节。

## 本轮修复完成

认证错误帧、PIPE nonce 重传策略、绝对会话期限已修；真实 SDK 加密全刷/局刷与原配置恢复通过。加密路径栈溢出已修，总 RAM 不变。新增致命错误自动重启及主循环硬件看门狗，最终构建的关中断死循环故障注入、硬件复位原因和 BLE 重连均通过。详细范围和未覆盖项见上述两份记录。
